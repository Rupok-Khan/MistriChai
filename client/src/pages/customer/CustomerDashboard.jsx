          ...
          <div className="admin-main">
            ...
            {activeSection === "overview" && (
              ...
              <DashboardInsights title="Order progress" subtitle="Your service journey by status" segments={[
                ...
                {activeSection === "subscription" && <div className="eco-card p-4"><h5 className="fw-bold">Booking Fee Subscription</h5><p className="small-muted">Choose a plan to make booking fees ৳0 during the active period. Final technician payment is separate; platform commission is 4% of that final amount.</p>{me.subscription ? <div className="alert alert-success">Active {me.subscription.plan_code === "ONE_YEAR" ? "1-year" : me.subscription.plan_code === "SIX_MONTHS" ? "6-month" : "1-month"} plan · valid until <b>{new Date(me.subscription.expires_at).toLocaleDateString()}</b>.</div> : <form onSubmit={subscribe} className="row g-3"><div className="col-12"><div className="row g-3">{[["ONE_MONTH","1 Month","৳30"],["SIX_MONTHS","6 Months","৳149"],["ONE_YEAR","1 Year","৳250"]].map(([value,label,price]) => <div className="col-md-4" key={value}><button type="button" className={`pricing-plan-card w-100 text-start ${subscriptionPlan === value ? "selected" : ""}`} onClick={() => setSubscriptionPlan(value)}><span>{label}</span><strong>{price}</strong><small>Booking fee waived</small></button></div>)}</div></div><div className="col-md-6"><label className="form-label">bKash Transaction ID</label><input className="form-control" value={subscriptionTrx} onChange={(e) => setSubscriptionTrx(e.target.value)} minLength="6" maxLength="50" required /></div><div className="col-12"><div className="alert alert-info">Send the selected amount to bKash <b>01984646174</b>, then submit the transaction ID.</div></div><div className="col-12"><button className="btn eco-btn">Activate Subscription</button></div></form>}</div>}
